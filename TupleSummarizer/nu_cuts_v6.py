...
if len(sys.argv) < 3:
  print (" arguments are ")
  print (" python nu_cuts_v5.py <json config file>  <input list of root files> [<input limits file>]")
...
inputlist = open(sys.argv[2],'r').readlines()
...
print ("the cuts are", thecut)

outname = sys.argv[2].replace(".txt","")
outname += "__"+sys.argv[1].replace(".json","")
outname = os.path.basename(outname)

# if you used a limits file, include that in the output filename

if haslim:
  #outname = outname+"__"+tag+"__"+ os.path.basename(sys.argv[3]).replace(".txt","")
  outname = outname+"___"+ os.path.basename(sys.argv[3]).replace(".txt","")
  outname += "_"
#else:
#  outname = outname#+"__"+tag
...
rangesname="ranges__"+outname
listing = open(rangesname+"__"+tuplename+".txt",'w')

# make the output histograms
histoutname = "hist__"+outname
hout = ROOT.TFile(histoutname+"__"+tuplename+".root","RECREATE")
